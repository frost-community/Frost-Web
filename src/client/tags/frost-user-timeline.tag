<frost-user-timeline>
	...
	<script>
		this.timelinePosts = [];
		this.loading = true;
		this.userId = opts.dataUserId;

		this.on('mount', () => {
			const endpoint = `/users/${this.userId}/timelines/user`;
				...
				if (rest.request.endpoint == endpoint) {
					if (rest.success) {
						if (rest.response.posts != null) {
							...
							alert(`api error: failed to fetch user timeline posts. ${rest.response.message}`);
						}
					}
					else {
					...
					// this.webSocket.sendEvent('timeline-connect', {type: 'user'});
			...
			this.webSocket.on('data:user:status', status => {
